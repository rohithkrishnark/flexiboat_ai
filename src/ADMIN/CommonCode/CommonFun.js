...
export const fetchFacDocumentFiles = async (id) => {
  try {
    const response = await axiosLogin.get(`/student/documents/faculty/${id}`);
    const { success, data } = response.data;
...
export const fetchFacDocumentDetails = async (id) => {
  try {
    const response = await axiosLogin.post(`/student/documents/by-user`, {
      uploaded_by: id,
    });
    const { success, data } = response.data;
...
export const fetchAllDocuemntDetail = async (id) => {
  try {
    const response = await axiosLogin.post(`/student/documents/by-dep`, {
      department_id: id,
    });
    const { success, data } = response.data;
...
export const fetchAllAlertNotify = async (logged_role) => {
  if (!logged_role) return warningNotify("Id and Role is Missing!");
  try {
    const response = await axiosLogin.post(`/alertnotify/getfacnotify`, {
      role: logged_role,
    });
...
export const fetchAllEnquiries = async () => {
  try {
    const response = await axiosLogin.get(`/training/alert/enquiry`);
    const { success, data, message } = response.data;
    if (success === 0) {
      errorNotify(message);
      return [];
